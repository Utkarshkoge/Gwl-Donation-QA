...
import { AppProvider as AppProvider$1, useIndexResourceState, IndexTable, Text, InlineStack, Button, Page, Layout, Banner, Box, List, Card, EmptyState, Badge, BlockStack, Divider, DescriptionList, Modal, Select, Icon } from "@shopify/polaris";
...
async function sendDonationReceipt({
  ...
  if (isRecurring && (type === "receipt" || type === "pause" || type === "resume" || type === "cancellation" || type === "reminder" || type === "recovery")) {
    ...
    const recurringBadge = isRecurringFreq ? `<div style="margin-bottom: 12px; padding: 8px 14px; background: #e8f5e9; border-radius: 6px; display: inline-block; font-size: 13px; color: #2e7d32;">
  ...
  try {
    ...
    if (contract) {
      ...
      if ((_k = contract.customer) == null ? void 0 : _k.email) {
        ...
        const frequencyVal = (recLog == null ? void 0 : recLog.frequency) === "weekly" ? "Weekly" : (recLog == null ? void 0 : recLog.frequency) === "daily" ? "Daily" : "Monthly";
...
function mapFrequencyLabel(freq) {
  if (freq === "daily") return "Daily";
  if (freq === "monthly") return "Monthly";
  if (freq === "weekly") return "Weekly";
  return "One-time";
}
function mapDonationType(source, frequency) {
  switch (source) {
    case "recurring": {
    ...
    case "pos":
    default:
      return "Portion of Sale";
  }
}
function normalizeOrderIdToNumeric(orderId) {
  if (!orderId) return "";
...
async function fetchDonationsByOrderIds(orderIds, shop) {
  ...
  const normalized = [
    ...posLogs.map((l) => normalizeDonationRecord(l, "pos")),
    ...recurringLogs.map((l) => normalizeDonationRecord(l, "recurring")),
    ...roundupLogs.map((l) => normalizeDonationRecord(l, "roundup")),
    ...presetDonations.map((d) => normalizeDonationRecord(d, "preset"))
  ];
  normalized.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
  return normalized;
}
function normalizeDonationRecord(record, source) {
  var _a2;
  if (source === "preset") {
    return {
      id: record.id,
      ...
      frequency: "One-time",
      campaignName: ((_a2 = record.campaign) == null ? void 0 : _a2.name) || "Donation",
  ...
  return {
    ...
    campaignName: "",
    createdAt: record.createdAt,
    subscriptionContractId: source === "recurring" ? record.subscriptionContractId || void 0 : void 0
  };
}
async function findDonationById(id) {
  ...
  record = await prisma.recurringDonationLog.findUnique({ where: { id } });
  if (record) return { record, source: "recurring" };
  record = await prisma.roundUpDonationLog.findUnique({ where: { id } });
...
async function generateReceiptPDF$1(args) {
  return new Promise((resolve, reject) => {
    ...
    doc.fontSize(28).font("Helvetica-Bold").fillColor("#FFFFFF").text("DONATION RECEIPT", 50, 40, { width: pageWidth });
    doc.fontSize(11).font("Helvetica").fillColor("#E8D5EF").text(args.shopName.charAt(0).toUpperCase() + args.shopName.slice(1), 50, 78, { width: pageWidth });
    doc.fontSize(10).fillColor("#E8D5EF").text(`Date: ${formattedDate}`, 50, 94, { width: pageWidth, align: "right" });
    let y = 140;
    doc.rect(50, y, pageWidth, 36).fill(lightGray);
    doc.fontSize(10).font("Helvetica-Bold").fillColor(darkColor).text(`Order: ${args.orderNumber}`, 62, y + 11).text(`Type: ${args.donationType}`, 300, y + 11);
    y += 56;
    doc.fontSize(12).font("Helvetica-Bold").fillColor(brandColor).text("DONOR INFORMATION", 50, y);
    y += 20;
    doc.moveTo(50, y).lineTo(50 + pageWidth, y).strokeColor("#E0E0E0").lineWidth(1).stroke();
    y += 12;
    const addInfoRow = (label2, value) => {
      doc.fontSize(10).font("Helvetica-Bold").fillColor(grayColor).text(label2, 50, y, { width: 140 });
    ...
    doc.moveTo(50, y).lineTo(50 + pageWidth, y).strokeColor("#E0E0E0").lineWidth(1).stroke();
    y += 20;
    doc.fontSize(14).font("Helvetica-Bold").fillColor(brandColor).text("Thank you for your generous donation!", 50, y, { width: pageWidth, align: "center" });
    y += 24;
    doc.fontSize(9).font("Helvetica").fillColor(grayColor).text("This receipt is for your records. Please retain it for tax purposes. If you have any questions about your donation, please contact us.", 50, y, { width: pageWidth, align: "center" });
    y += 30;
    doc.fontSize(8).font("Helvetica").fillColor("#AAAAAA").text(`Generated on ${(/* @__PURE__ */ new Date()).toLocaleDateString("en-US", { year: "numeric", month: "long", day: "numeric" })}`, 50, y, { width: pageWidth, align: "center" });
    doc.end();
  });
}
const loader$B = async ({
  ...
  if (!donationId) {
    return new Response("Missing donation ID", {
      status: 400
    });
  }
  let admin;
  ...
  if (!admin) {
    return new Response("Unauthorized: Store connection not found", {
      status: 401
    });
  }
  if (!customerId) {
    return new Response("Unauthorized: You must be logged in to download receipts", {
      status: 401
    });
  }
  ...
  try {
    ...
    if (!result) {
      ...
      return new Response("Donation record not found", {
        status: 404
      });
    }
    const {
    ...
    if (!orderId) {
      ...
      return new Response("No order associated with this donation", {
        status: 400
      });
    }
    ...
    try {
      ...
      orderData = (_a2 = json.data) == null ? void 0 : _a2.order;
      if (!orderData) {
      ...
      const orderCustomerId = (_b = orderData.customer) == null ? void 0 : _b.id;
      if (orderCustomerId !== customerGid) {
        console.warn("[CustomerReceiptDownload] Customer ID mismatch!", {
          expected: customerGid,
          actual: orderCustomerId,
          orderId: orderGid
        });
        return new Response("Access denied: This receipt does not belong to your account", {
          status: 403
        });
      }
    } catch (gqlError) {
      console.error("[CustomerReceiptDownload] GraphQL order fetch failed:", gqlError);
      return new Response("Could not verify order ownership", {
        status: 500
      });
    }
    const customerName = orderData.billingAddress ? `${orderData.billingAddress.firstName || ""} ${orderData.billingAddress.lastName || ""}`.trim() : "";
    let shippingAddress = "";
    ...
    const pdfBuffer = await generateReceiptPDF$1({
    ...
    return new Response("Failed to generate receipt: " + error.message, {
      status: 500
    });
  }
};
...
${order.billingAddress.country}` : "";
      ...
      const res = await sendDonationReceipt({
        ...
        shop,
        donationName: presetDonation ? (_g = presetDonation.campaign) == null ? void 0 : _g.name : productTitleForEmail,
        frequency: freqLabel,
        shippingAddress: shippingAddr,
        billingAddress: billingAddr,
        productTitle: productTitleForEmail,
        manageUrl: freqLabel !== "One-time" ? `https://${shop}/account/subscriptions` : void 0,
        nextBillingDate
      });
      if (res.success) {
        if (logType === "pos" && log) {
...
const action$k = async ({
  ...
  try {
    ...
    if (appSettings && !appSettings.enabled) {
      return jsonResp({
        success: false,
        error: "App is currently disabled"
      }, 400);
    }
  } catch (err) {
...
[!!! WEBHOOK HIT !!!] Incoming request to /webhooks/orders/create at ${(/* @__PURE__ */ new Date()).toISOString()}`);
  ...
  try {
    ...
    if (topic === "ORDERS_CREATE" && order.financial_status === "pending") {
      ...
      return new Response("OK - pending payment, awaiting orders/paid", {
        status: 200
      });
    }
    const appSettings = await prisma.appSettings.findUnique({
    ...
    const isAppEnabled = (appSettings == null ? void 0 : appSettings.enabled) ?? true;
    if (!isAppEnabled) {
      console.log(`[Webhook] App is globally disabled for shop ${shop}. Skipping donation processing for Order ${order.name}.`);
      return new Response("OK - app globally disabled", {
        status: 200
      });
    }
    ...
    let hasCampaignRecurring = false;
    let campaignRecurringNames = [];
        ...
        try {
          ...
          if (isDonation && matchingCampaign) {
            ...
            if (hasSellingPlan || hasSellingPlanProp) {
              const basePrice2 = parseFloat(item.price || "0") * (item.quantity || 1);
    ...
    if (isRecurring && recurringSellingPlanId) {
      ...
      if (config) {
        ...
        else if ((_h = config.dailyPlanId) == null ? void 0 : _h.includes(spId)) frequency = "daily";
      }
    }
    for (const lineItem of order.line_items || []) {
      const spProp = (lineItem.properties || []).find((p) => p.name === "selling_plan" || p.name === "_selling_plan_id");
...
${order.billing_address.country}` : "";
        let donationItem = (order.line_items || []).find((li) => String(li.product_id) === String(DONATION_PRODUCT_ID));
        if (!donationItem && hasCampaignDonation) {
          donationItem = (order.line_items || []).find((li) => {
            var _a3, _b2;
            return li.properties && ((_b2 = (_a3 = li.properties).some) == null ? void 0 : _b2.call(_a3, (p) => p.name === "Donation Campaign"));
          }) || (order.line_items || []).find((li) => {
        ...
        const resolvedProductTitle = (donationItem == null ? void 0 : donationItem.title) || campaignRecurringNames[0] || directDonationName;
        ...
        const res = await sendDonationReceipt({
          ...
          manageUrl: frequency !== "one_time" ? `https://${shop}/apps/pos-donation-1/subscriptions` : void 0,
          nextBillingDate,
          paymentMethod,
          productImage: order.donation_product_image,
          // Pass all donation line items for multi-product email
          lineItems: donationLineItems.length > 1 ? donationLineItems : void 0
        });
        ...
        if (hasDirectDonationProduct && frequency === "one_time" && !isSubscriptionRenewal) {
          try {
            ...
            if (donationItem && donationItem.properties) {
              const prop = donationItem.properties.find((p) => p.name === "Donation Campaign");
              if (prop) {
            ...
            if (campaign) {
              const variantIdStr = ((_r = donationItem == null ? void 0 : donationItem.variant_id) == null ? void 0 : _r.toString()) || "unknown";
              await prisma.donation.upsert({
                where: {
                  orderId_shopifyVariantId: {
                    orderId,
      ...
      if (admin) {
        try {
          ...
          if (hasCampaignDonation && !isSubscriptionRenewal) {
            ...
            const customerId = order.customer_gql_id || (((_t = order.customer) == null ? void 0 : _t.id) ? `gid://shopify/Customer/${order.customer.id}` : null);
            if (customerId) {
              await admin.graphql(`#graphql
                                mutation tagsAdd($id: ID!, $tags: [String!]!) { tagsAdd(id: $id, tags: $tags) { node { id } } }`, {
                variables: {
...
const loader$x = async ({
  ...
  if (!logId && !donationId) {
    return new Response("Missing logId or donationId parameter", {
      status: 400
    });
  }
  let admin, session;
  ...
  try {
    ...
    if (donationId) {
      ...
      if (!donation) {
        return new Response("Donation not found", {
          status: 404
        });
      }
      ...
      donationType = "Preset";
      campaignName = ((_a2 = donation.campaign) == null ? void 0 : _a2.name) || "Donation";
      createdDate = donation.createdAt;
    } else if (logId) {
      let log = null;
      let logType = "pos";
      log = await prisma.posDonationLog.findUnique({
        where: {
          id: logId
        }
      });
      ...
      let orderIdForQuery = presetDonation ? presetDonation.orderId : log == null ? void 0 : log.orderId;
      ...
      if (orderIdForQuery) {
        try {
          ...
          const orderData = await orderResponse.json();
          const order = (_b = orderData.data) == null ? void 0 : _b.order;
          if (order) {
            orderNumber = order.name || orderNumber;
            customerEmail = order.email || "";
            customerName = order.billingAddress ? `${order.billingAddress.firstName || ""} ${order.billingAddress.lastName || ""}`.trim() : "";
      ...
      if (presetDonation) {
        ...
        donationType = "Preset";
        campaignName = ((_c = presetDonation.campaign) == null ? void 0 : _c.name) || "Preset Donation";
...
async function generateReceiptPDF(args) {
  return new Promise((resolve, reject) => {
    const doc = new PDFDocument({
      size: "A4",
      margin: 50,
    ...
    doc.rect(0, 0, doc.page.width, 120).fill(brandColor);
    doc.fontSize(28).font("Helvetica-Bold").fillColor("#FFFFFF").text("DONATION RECEIPT", 50, 40, {
      width: pageWidth
    });
    doc.fontSize(11).font("Helvetica").fillColor("#E8D5EF").text(args.shopName.charAt(0).toUpperCase() + args.shopName.slice(1), 50, 78, {
      width: pageWidth
    });
    doc.fontSize(10).fillColor("#E8D5EF").text(`Date: ${formattedDate}`, 50, 94, {
      width: pageWidth,
      align: "right"
    });
    let y = 140;
    ...
    y += 12;
    const addInfoRow = (label2, value) => {
      doc.fontSize(10).font("Helvetica-Bold").fillColor(grayColor).text(label2, 50, y, {
        width: 140
      });
      doc.fontSize(10).font("Helvetica").fillColor(darkColor).text(value || "N/A", 200, y, {
        width: pageWidth - 150
      });
      y += 20;
    };
    ...
    if (args.billingAddress || args.shippingAddress) {
      ...
      if (args.billingAddress) {
        doc.fontSize(9).font("Helvetica-Bold").fillColor(grayColor).text("BILLING ADDRESS", 50, y);
        doc.fontSize(10).font("Helvetica").fillColor(darkColor).text(args.billingAddress, 50, y + 14, {
          width: colWidth
        });
      }
      if (args.shippingAddress) {
        doc.fontSize(9).font("Helvetica-Bold").fillColor(grayColor).text("SHIPPING ADDRESS", 50 + colWidth + 20, y);
        doc.fontSize(10).font("Helvetica").fillColor(darkColor).text(args.shippingAddress, 50 + colWidth + 20, y + 14, {
          width: colWidth
        });
      }
      y += 80;
    ...
    doc.moveTo(50, y).lineTo(50 + pageWidth, y).strokeColor("#E0E0E0").lineWidth(1).stroke();
    y += 20;
    doc.fontSize(14).font("Helvetica-Bold").fillColor(brandColor).text("Thank you for your generous donation!", 50, y, {
      width: pageWidth,
      align: "center"
    });
    y += 24;
    doc.fontSize(9).font("Helvetica").fillColor(grayColor).text("This receipt is for your records. Please retain it for tax purposes. If you have any questions about your donation, please contact us.", 50, y, {
      width: pageWidth,
      align: "center"
    });
    y += 30;
    doc.fontSize(8).font("Helvetica").fillColor("#AAAAAA").text(`Generated on ${(/* @__PURE__ */ new Date()).toLocaleDateString("en-US", {
      year: "numeric",
      month: "long",
      day: "numeric"
    })}`, 50, y, {
...
const loader$u = async ({
  ...
  try {
    ...
    for (const {
      ...
      try {
        ...
        const contracts = ((_b = (_a2 = json.data) == null ? void 0 : _a2.subscriptionContracts) == null ? void 0 : _b.edges) || [];
        const dueContracts = contracts.filter((edge) => {
        ...
        for (const edge of dueContracts) {
          ...
          try {
            const line = (_e = (_d = (_c = contract.lines) == null ? void 0 : _c.edges) == null ? void 0 : _d[0]) == null ? void 0 : _e.node;
            const sellingPlanName = ((line == null ? void 0 : line.sellingPlanName) || "").toLowerCase();
            let dedupHours = 20;
            ...
            const recentAttempt = await prisma.billingAttemptLog.findFirst({
              where: {
                shop,
                subscriptionContractId: contractId,
                source: "cron_billing",
                ...
                createdAt: {
                  gte: new Date(now.getTime() - dedupHours * 60 * 60 * 1e3)
                }
              }
            });
            if (recentAttempt) {
            ...
            const activeRecovery = await prisma.paymentRecoveryLog.findUnique({
              where: {
                shop_subscriptionContractId: {
                  shop,
                  subscriptionContractId: contractId
            ...
            const amount = parseFloat(((_f = line == null ? void 0 : line.currentPrice) == null ? void 0 : _f.amount) || "0");
            ...
            const billingJson = await billingResponse.json();
            const billingResult = (_g = billingJson.data) == null ? void 0 : _g.subscriptionBillingAttemptCreate;
            if (((_h = billingResult == null ? void 0 : billingResult.userErrors) == null ? void 0 : _h.length) > 0) {
              const errMsg = billingResult.userErrors.map((e) => e.message).join("; ");
            ...
            const billingAttempt = billingResult == null ? void 0 : billingResult.subscriptionBillingAttempt;
            const billingAttemptId = (billingAttempt == null ? void 0 : billingAttempt.id) || null;
            results.billingTriggered++;
            if (billingAttempt == null ? void 0 : billingAttempt.order) {
            ...
            } else if ((billingAttempt == null ? void 0 : billingAttempt.errorCode) || (billingAttempt == null ? void 0 : billingAttempt.errorMessage)) {
        ...
        if ((_i = shopErr.message) == null ? void 0 : _i.includes("Could not find a shop")) {
          continue;
        }
  ...
  console.log(`[CronBilling] ─── Complete in ${elapsed}ms. Shops: ${results.shopsProcessed}, Due: ${results.contractsFound}, Triggered: ${results.billingTriggered}, Success: ${results.billingSucceeded}, Pending: ${results.billingPending}, Failed: ${results.billingFailed}, Skipped: ${results.skipped} ───`);
  return new Response(JSON.stringify({
    success: true,
    ...results,
    elapsedMs: elapsed,
...
const loader$r = async ({
  ...
  if (!shop || !contractId) {
    return new Response("Missing required parameters: shop, contractId", {
      status: 400
    });
  }
  const normalizedContractId = contractId.startsWith("gid://") ? contractId : `gid://shopify/SubscriptionContract/${contractId}`;
  ...
  try {
    ...
    const json = await response.json();
    const contractDetails = (_a2 = json.data) == null ? void 0 : _a2.subscriptionContract;
    if (!contractDetails) {
      return new Response(`Subscription contract ${normalizedContractId} not found on Shopify for shop ${shop}`, {
        status: 404
      });
    }
    const customer = contractDetails.customer;
    const line = (_d = (_c = (_b = contractDetails.lines) == null ? void 0 : _b.edges) == null ? void 0 : _c[0]) == null ? void 0 : _d.node;
    ...
    if (recoveryLog.status === "exhausted") {
      ...
      if (fallbackAction === "pause" || fallbackAction === "cancel") {
        try {
          ...
          const result = (_f = fallbackJson.data) == null ? void 0 : _f[mutationName];
          if (((_g = result == null ? void 0 : result.userErrors) == null ? void 0 : _g.length) > 0) {
    ...
    return new Response(`TestFailure Error: ${err.message || err}`, {
      status: 500
    });
  }
};
    ...
    if (json.errors) {
      ...
      return liquid(`
                <div style="max-width:700px;margin:40px auto;font-family:sans-serif;padding:0 16px;">
                    ...
                    <div style="background:#fee2e2;border:1px solid #f87171;color:#991b1b;padding:16px;border-radius:8px;">
                        <p style="margin:0;font-weight:bold;">${((_a2 = json.errors[0]) == null ? void 0 : _a2.message) || "Unknown error"}</p>
                    </div>
                </div>
            `);
    }
    const orders = ((_e = (_d = (_c = (_b = json == null ? void 0 : json.data) == null ? void 0 : _b.customer) == null ? void 0 : _c.orders) == null ? void 0 : _d.edges) == null ? void 0 : _e.map((e) => e.node)) ?? [];
    contracts = ((_i = (_h = (_g = (_f = json == null ? void 0 : json.data) == null ? void 0 : _f.customer) == null ? void 0 : _g.subscriptionContracts) == null ? void 0 : _h.edges) == null ? void 0 : _i.map((e) => e.node)) ?? [];
    for (const order of orders) {
      const numericId = normalizeOrderIdToNumeric(order.id);
      orderIds.push(numericId);
      orderMap.set(numericId, {
        name: order.name,
        email: order.email || ""
      });
    ...
    for (const donation of donations) {
      ...
      if (donation.donationSource === "recurring") {
        const matchingContract = contracts.find((c) => {
          ...
          const contractOrderId = ((_a3 = c.originOrder) == null ? void 0 : _a3.id) ? c.originOrder.id.replace("gid://shopify/Order/", "").trim() : "";
          ...
          const contractOrderName = ((_b2 = c.originOrder) == null ? void 0 : _b2.name) || "";
        ...
        if (matchingContract) {
          const lines = ((_k = (_j = matchingContract.lines) == null ? void 0 : _j.edges) == null ? void 0 : _k.map((e) => e.node)) ?? [];
          const productTitles = lines.map((l) => `${l.quantity > 1 ? l.quantity + "× " : ""}${l.title}`).join(", ");
          if (productTitles) {
    ...
    children: /* @__PURE__ */ jsxs(AppProvider$1, {
      ...
      children: [/* @__PURE__ */ jsxs("ui-nav-menu", {
        children: [/* @__PURE__ */ jsx(Link, {
          ...
          to: "/app/oneday-subscription",
          children: "Daily Donation"
        }), /* @__PURE__ */ jsx(Link, {
...
async function setupSellingPlans(admin, shop, productId, retryCount = 0) {
  ...
  let dailyPlanId = existingConfig == null ? void 0 : existingConfig.dailyPlanId;
  ...
  const oneDayConfig = await prisma.oneDayDonationConfig.findUnique({
    where: { shop }
  });
  const isDailyActive = (oneDayConfig == null ? void 0 : oneDayConfig.isActive) ?? false;
  if (!sellingPlanGroupId) {
    ...
    dailyPlanId = ((_h = plans.find((p) => p.name.toLowerCase().includes("daily"))) == null ? void 0 : _h.id) || null;
  }
  ...
  const attachErrors = ((_j = (_i = attachData.data) == null ? void 0 : _i.sellingPlanGroupAddProducts) == null ? void 0 : _j.userErrors) || [];
  ...
  if (variantIds.length > 0 && sellingPlanGroupId) {
    ...
    const variantErrors = ((_l = (_k = attachVariantsData.data) == null ? void 0 : _k.sellingPlanGroupAddProductVariants) == null ? void 0 : _l.userErrors) || [];
  ...
  if (dailyPlanId) {
    await prisma.oneDayDonationConfig.upsert({
      where: { shop },
      update: {
        dailyPlanId
      },
...
${addr.country}` : "N/A";
      ...
      const emailResult = await sendDonationReceipt({
        ...
        type: emailType,
        shop,
        frequency: (log == null ? void 0 : log.frequency) === "monthly" ? "Monthly" : (log == null ? void 0 : log.frequency) === "weekly" ? "Weekly" : (log == null ? void 0 : log.frequency) === "daily" ? "Daily" : "One-time",
        ...
        children: /* @__PURE__ */ jsx(Card, {
          ...
          children: contracts && contracts.length > 0 ? /* @__PURE__ */ jsx(IndexTable, {
            ...
            action: {
              ...
              onAction: () => navigate("/app/preset-donation")
            },
...
async function setupOneDaySellingPlan(admin, shop) {
  var _a2, _b, _c, _d, _e, _f, _g, _h, _i, _j, _k;
  const recurringConfig = await prisma.recurringDonationConfig.findUnique({
    where: { shop }
  });
  ...
  if (recurringConfig.dailyPlanId) {
    ...
    try {
      ...
      const plans = ((_d = (_c = (_b = (_a2 = verifyJson.data) == null ? void 0 : _a2.sellingPlanGroup) == null ? void 0 : _b.sellingPlans) == null ? void 0 : _c.edges) == null ? void 0 : _d.map((e) => e.node)) ?? [];
      const existingDaily = plans.find(
        (p) => p.id === recurringConfig.dailyPlanId || p.name.toLowerCase().includes("daily")
      );
      if (existingDaily) {
  ...
  const userErrors = ((_f = (_e = updateJson.data) == null ? void 0 : _e.sellingPlanGroupUpdate) == null ? void 0 : _f.userErrors) || [];
  if (userErrors.length > 0) {
    throw new Error(
      `Failed to create Daily Donation plan: ${userErrors.map((e) => e.message).join(", ")}`
    );
  }
  const updatedPlans = ((_k = (_j = (_i = (_h = (_g = updateJson.data) == null ? void 0 : _g.sellingPlanGroupUpdate) == null ? void 0 : _h.sellingPlanGroup) == null ? void 0 : _i.sellingPlans) == null ? void 0 : _j.edges) == null ? void 0 : _k.map(
    (e) => e.node
  )) ?? [];
  const dailyPlan = updatedPlans.find(
    (p) => p.name.toLowerCase().includes("daily")
  );
  if (!dailyPlan) {
...
async function removeOneDaySellingPlan(admin, shop) {
  var _a2, _b;
  const recurringConfig = await prisma.recurringDonationConfig.findUnique({
    where: { shop }
  });
  if (!(recurringConfig == null ? void 0 : recurringConfig.sellingPlanGroupId) || !(recurringConfig == null ? void 0 : recurringConfig.dailyPlanId)) {
  ...
  try {
    ...
    const json = await response.json();
    const errors = ((_b = (_a2 = json.data) == null ? void 0 : _a2.sellingPlanGroupUpdate) == null ? void 0 : _b.userErrors) || [];
    if (errors.length > 0) {
...
async function getOneDayConfig(shop) {
  return prisma.oneDayDonationConfig.findUnique({ where: { shop } });
}
async function syncOneDayConfig(shop, dailyPlanId, sellingPlanGroupId) {
  await prisma.recurringDonationConfig.update({
    where: { shop },
    data: { dailyPlanId }
  });
...
async function clearOneDayConfig(shop) {
  try {
    await prisma.recurringDonationConfig.update({
      where: { shop },
...
const loader$f = async ({
  ...
  return {
    isActive: (oneDayConfig == null ? void 0 : oneDayConfig.isActive) ?? false,
    dailyPlanId: (oneDayConfig == null ? void 0 : oneDayConfig.dailyPlanId) ?? (recurringConfig == null ? void 0 : recurringConfig.dailyPlanId) ?? null,
    hasRecurringSetup: !!(recurringConfig == null ? void 0 : recurringConfig.sellingPlanGroupId),
    sellingPlanGroupId: (recurringConfig == null ? void 0 : recurringConfig.sellingPlanGroupId) ?? null
  };
};
...
const app_onedaySubscription = UNSAFE_withComponentProps(function OneDaySubscriptionPage() {
  var _a2, _b, _c, _d;
  const {
    isActive,
    dailyPlanId,
  ...
  const isSubmitting = fetcher.state === "submitting";
  const currentlyActive = ((_a2 = fetcher.data) == null ? void 0 : _a2.success) && ((_b = fetcher.data) == null ? void 0 : _b.dailyPlanId) ? true : ((_c = fetcher.data) == null ? void 0 : _c.success) && !((_d = fetcher.data) == null ? void 0 : _d.dailyPlanId) ? false : isActive;
  return /* @__PURE__ */ jsx(Page, {
    ...
    children: /* @__PURE__ */ jsxs(Layout, {
      children: [fetcher.data && !fetcher.data.success && !isSubmitting && /* @__PURE__ */ jsx(Layout.Section, {
        children: /* @__PURE__ */ jsx(Banner, {
          tone: "critical",
          title: "Error",
          children: /* @__PURE__ */ jsx("p", {
            ...
            children: [/* @__PURE__ */ jsx(InlineStack, {
              ...
              children: /* @__PURE__ */ jsxs(BlockStack, {
                ...
                children: [/* @__PURE__ */ jsxs(InlineStack, {
                  ...
                  children: [/* @__PURE__ */ jsx(Text, {
                    variant: "headingMd",
                    as: "h2",
                    children: "Daily Donation Subscription"
                  }), currentlyActive ? /* @__PURE__ */ jsx(Badge, {
                    tone: "success",
                    children: "Active"
                  }) : /* @__PURE__ */ jsx(Badge, {
                    tone: "enabled",
                  ...
                  tone: "subdued",
                  as: "p",
                  children: 'When enabled, a "Daily Donation" option appears alongside Weekly and Monthly in the recurring donation frequency dropdown on your storefront. Customers who select this plan will be billed every day.'
                })]
              })
              ...
              children: [/* @__PURE__ */ jsxs(InlineStack, {
                ...
                children: [/* @__PURE__ */ jsx(Text, {
                  ...
                  variant: "bodyMd",
                  as: "span",
                  children: "Every day (DAY interval, count 1)"
                })]
                ...
                children: [/* @__PURE__ */ jsx(Text, {
                  ...
                  fontWeight: "semibold",
                  as: "span",
                  children: "Selling Plan Group:"
                }), /* @__PURE__ */ jsx(Text, {
                  variant: "bodyMd",
                  as: "span",
                  children: sellingPlanGroupId ? `Recurring Donations (${sellingPlanGroupId.split("/").pop()})` : "Not configured"
                })]
              }), dailyPlanId && currentlyActive && /* @__PURE__ */ jsxs(InlineStack, {
                gap: "200",
                blockAlign: "center",
                children: [/* @__PURE__ */ jsx(Text, {
                  variant: "bodyMd",
                  ...
                  tone: "subdued",
                  as: "span",
                  children: dailyPlanId
                })]
              })]
            }), /* @__PURE__ */ jsx(Divider, {}), /* @__PURE__ */ jsx(InlineStack, {
              align: "end",
              gap: "200",
              children: currentlyActive ? /* @__PURE__ */ jsx(Button, {
                tone: "critical",
                loading: isSubmitting,
                disabled: isSubmitting,
                onClick: () => {
                  if (confirm("Are you sure you want to disable Daily Donation? The plan will be removed from Shopify and will no longer appear on the storefront.")) {
                    fetcher.submit({
                      _action: "disable"
                    }, {
                      method: "POST"
                ...
                children: "Disable Daily Donation"
              }) : /* @__PURE__ */ jsx(Button, {
                variant: "primary",
                loading: isSubmitting,
                disabled: isSubmitting,
                ...
                children: "Enable Daily Donation"
              })
            })]
          })
        })
          ...
          children: /* @__PURE__ */ jsxs(BlockStack, {
            ...
            children: [/* @__PURE__ */ jsx(Text, {
              ...
              children: [/* @__PURE__ */ jsxs(Text, {
                ...
                children: [/* @__PURE__ */ jsx("strong", {
                  children: "1."
                }), ' Enable the Daily Donation plan above. This adds a "Daily Donation" selling plan to your existing Recurring Donations group in Shopify.']
              }), /* @__PURE__ */ jsxs(Text, {
                variant: "bodyMd",
                as: "p",
                children: [/* @__PURE__ */ jsx("strong", {
                  children: "2."
                }), ' On your storefront, the "Daily Donation" option will automatically appear in the frequency dropdown alongside Weekly and Monthly.']
              }), /* @__PURE__ */ jsxs(Text, {
                variant: "bodyMd",
                as: "p",
                children: [/* @__PURE__ */ jsx("strong", {
                  children: "3."
                }), ' When a customer selects "Daily Donation" and completes checkout, Shopify creates a subscription contract that bills every day.']
              }), /* @__PURE__ */ jsxs(Text, {
                variant: "bodyMd",
                as: "p",
                children: [/* @__PURE__ */ jsx("strong", {
                  children: "4."
                }), " All existing subscription management features (pause, resume, cancel, payment recovery) work automatically with daily subscriptions."]
              })]
            })]
          })
        })
...
const route37 = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  action: action$9,
  default: app_onedaySubscription,
  loader: loader$f
}, Symbol.toStringTag, { value: "Module" }));
...
const action$8 = async ({
  ...
  try {
    ...
    if (actionType === "trigger_billing_attempt") {
      ...
      const json = await response.json();
      const errors = (_d = (_c = json == null ? void 0 : json.data) == null ? void 0 : _c.subscriptionBillingAttemptCreate) == null ? void 0 : _d.userErrors;
      if (errors && errors.length > 0) {
        throw new Error(errors[0].message);
      }
      attempt = (_f = (_e = json == null ? void 0 : json.data) == null ? void 0 : _e.subscriptionBillingAttemptCreate) == null ? void 0 : _f.subscriptionBillingAttempt;
      billingAttemptId = (attempt == null ? void 0 : attempt.id) || `gid://shopify/SubscriptionBillingAttempt/${now.getTime()}`;
      let contractDetails = null;
      try {
        const contractResponse = await admin.graphql(`#graphql
        ...
        contractDetails = (_g = contractJson.data) == null ? void 0 : _g.subscriptionContract;
      } catch (gqlErr) {
        console.error(`[SubscriptionDetail] Failed to fetch contract details for manual log:`, gqlErr);
      }
      const db = (await Promise.resolve().then(() => db_server)).default;
      const customer = contractDetails == null ? void 0 : contractDetails.customer;
      const line = (_j = (_i = (_h = contractDetails == null ? void 0 : contractDetails.lines) == null ? void 0 : _h.edges) == null ? void 0 : _i[0]) == null ? void 0 : _j.node;
      const originOrder = contractDetails == null ? void 0 : contractDetails.originOrder;
      const amount = parseFloat(((_k = line == null ? void 0 : line.currentPrice) == null ? void 0 : _k.amount) || "0");
      const currency = (contractDetails == null ? void 0 : contractDetails.currencyCode) || "USD";
      const donationName = (line == null ? void 0 : line.title) || "Recurring Donation";
      const frequency = (line == null ? void 0 : line.sellingPlanName) || "Subscription";
      const customerEmail = (customer == null ? void 0 : customer.email) || "";
      ...
      if (attempt == null ? void 0 : attempt.errorCode) {
        ...
        if (recoverySettings == null ? void 0 : recoverySettings.enabled) {
            ...
            if (recoveryLog.status === "exhausted") {
              ...
              if (mutation) {
                try {
                  const fallbackResponse = await admin.graphql(mutation);
                  const fallbackJson = await fallbackResponse.json();
                  const fallbackResult = (_l = fallbackJson.data) == null ? void 0 : _l[mutationName];
                  if (((_m = fallbackResult == null ? void 0 : fallbackResult.userErrors) == null ? void 0 : _m.length) > 0) {
          ...
          if (recoverySettings.sendNotifications && customerEmail) {
            try {
              const {
                sendDonationReceipt: sendDonationReceipt2
              } = await Promise.resolve().then(() => sendgrid_server);
              await sendDonationReceipt2({
                email: customerEmail,
                name: customerName,
                amount: amount.toFixed(2),
                orderNumber: (originOrder == null ? void 0 : originOrder.name) || "N/A",
                type: recoveryLog.status === "exhausted" ? recoverySettings.fallbackAction === "cancel" ? "cancellation" : "pause" : "recovery",
                shop: session.shop,
                frequency: frequency.toLowerCase().includes("month") ? "Monthly" : frequency.toLowerCase().includes("week") ? "Weekly" : "Subscription",
                nextBillingDate: nextRetryDate.toLocaleDateString("en-US", {
                  month: "short",
      ...
      const orderId = ((_n = attempt == null ? void 0 : attempt.order) == null ? void 0 : _n.id) || null;
      const orderName = ((_o = attempt == null ? void 0 : attempt.order) == null ? void 0 : _o.name) || "";
      try {
        await db.billingAttemptLog.create({
          data: {
            shop: session.shop,
...
const app_subscriptionDetail = UNSAFE_withComponentProps(function SubscriptionDetailPage() {
  ...
  useEffect(() => {
    ...
    if ((_a3 = fetcher.data) == null ? void 0 : _a3.message) {
      ...
      shopify2.toast.show(toastMessage, {
        isError: !fetcher.data.success
      });
    }
  }, [fetcher.data, shopify2]);
  ...
  return /* @__PURE__ */ jsxs(Page, {
    ...
    children: [/* @__PURE__ */ jsxs(Layout, {
      ...
      }), fetcher.data && !isSubmitting && /* @__PURE__ */ jsx(Layout.Section, {
        children: /* @__PURE__ */ jsx(Banner, {
...
const app_donationActivity = UNSAFE_withComponentProps(function DonationActivity() {
  ...
  const handleDownload = useCallback(async (logId, source) => {
    ...
    shopify2.toast.show("Generating receipt PDF...");
    try {
      const response = await fetch(downloadUrl);
      if (!response.ok) {
        const errorText = await response.text();
        shopify2.toast.show(errorText || "Failed to generate receipt", {
          isError: true
        });
      ...
      shopify2.toast.show("Receipt downloaded!");
    } catch (error) {
      console.error("[DownloadReceipt] Error:", error);
      shopify2.toast.show("Failed to download receipt", {
        isError: true
      });
    } finally {
      setDownloadingId(null);
...
const action$7 = async ({
  ...
  if (plan !== "pro") {
    return {
      status: "error",
      message: "Failed Payment Recovery is only available on the Pro Plan."
    };
  }
...
const app_paymentRecovery = UNSAFE_withComponentProps(function PaymentRecoveryPage() {
  ...
  return /* @__PURE__ */ jsxs("s-page", {
    ...
    children: [/* @__PURE__ */ jsx("s-button", {
      ...
      disabled: isSaving,
      children: isProPlan ? isSaving ? "Saving..." : "Save Settings" : "Upgrade to Unlock ↗"
    }), /* @__PURE__ */ jsxs("div", {
      className: "recovery-settings-layout",
      children: [!isProPlan && /* @__PURE__ */ jsxs("div", {
        className: "plan-gate-banner",
        children: [/* @__PURE__ */ jsx("div", {
          className: "plan-gate-icon",
          children: "🔒"
        }), /* @__PURE__ */ jsxs("div", {
          className: "plan-gate-body",
          children: [/* @__PURE__ */ jsx("h3", {
            children: "Pro Plan Feature"
          }), /* @__PURE__ */ jsxs("p", {
            children: ["Failed Payment Recovery and Smart Retry Automation are available exclusively on the ", /* @__PURE__ */ jsx("strong", {
              children: "Pro Plan"
            }), "."]
          }), /* @__PURE__ */ jsx(Link, {
            to: "/app/pricing",
            className: "upgrade-link-btn",
            children: "Upgrade Plan to Unlock ↗"
          })]
        })]
      }), /* @__PURE__ */ jsxs("div", {
        ...
        children: [/* @__PURE__ */ jsxs("div", {
          ...
          children: [/* @__PURE__ */ jsx("h2", {
            ...
            children: /* @__PURE__ */ jsx("s-badge", {
              tone: enabled && isProPlan ? "success" : "caution",
              children: enabled && isProPlan ? "Currently Active" : "Currently Disabled"
            })
          })]
        }), /* @__PURE__ */ jsx("div", {
        ...
        className: `recovery-logic-container ${enabled && isProPlan ? "active" : "disabled"}`,
        children: [/* @__PURE__ */ jsxs("div", {
          className: "settings-row",
          children: [/* @__PURE__ */ jsxs("div", {
            className: "settings-info",
...
const app_posDonation = UNSAFE_withComponentProps(function PosDonation() {
  ...
  const renderConfiguration = () => /* @__PURE__ */ jsx(ConfigurationTab, {
    blocks: [{
      id: "pos-product",
      title: "Product Page Setup",
      description: "To add the donation section to your product page, click the button below to insert the app block.",
...
const routes = {
  ...
  "routes/api.customer.receipt-download": {
    id: "routes/api.customer.receipt-download",
    parentId: "root",
    path: "api/customer/receipt-download",
    index: void 0,
    caseSensitive: void 0,
    module: route8
  },
  ...
  "routes/app.oneday-subscription": {
    id: "routes/app.oneday-subscription",
    parentId: "routes/app",
    path: "oneday-subscription",
    index: void 0,
    caseSensitive: void 0,
    module: route37
  },
